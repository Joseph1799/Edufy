...
(function() {
  ...
  document.getElementById('loginForm').addEventListener('submit', function (event) {
    ...
    fetch('https://localhost:44370/api/' + (rol.toLowerCase() === 'estudiante' ? 'estudiante' : 'profesor') + '/login', { // Realiza la solicitud POST a la API
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ CorreoElectronico: email, Contrasenia: password, Rol: rol }) // Convierte los datos en formato JSON y los envía en el cuerpo de la solicitud
    })
      .then(response => {
        if (!response.ok) {
